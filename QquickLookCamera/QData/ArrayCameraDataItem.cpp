...
//只要丢包就丢弃
#include <QFile>
unsigned char* buf0 = new unsigned char[1024 * 1024 * 512];
int cnt = 0;
void ArrayCameraDataItem::storePayloadData(const unsigned char *buf)
{
	...
	if (frmCnt == m_newFrame && (lineCnt + 1 == curLineCnt || -1 == lineCnt))
	{
		...
		if (m_dualImageBuffer[0])
		{
			...
			cnt += lineSize + headerSize;

			if (cnt > 500 * 1024 * 1024)
			{
				static int fnum = 0;
				QFile f(QObject::tr("C:/recv_%1.dat").arg(++fnum)); f.open(QIODevice::WriteOnly); f.write((char*)buf0, cnt); f.close();
			}
		}
