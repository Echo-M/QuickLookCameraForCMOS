...
#include <QHBoxLayout>
#include <QVBoxLayout>
MyClass::MyClass(int _cmosId, int _angle, float _ratio, QWidget *parent, Qt::WindowFlags f)
	: QWidget(parent, f), 
	...
	m_BufPicNum(64)
{
	...
	int width = m_imgWidth*abs(fcos) + m_height*abs(fsin);
	if (width % 4 != 0) width += 4 - width % 4;
	int height = m_imgWidth*abs(fsin) + m_height*abs(fcos);
	...
	std::dynamic_pointer_cast<CCirQueue>(m_cmosRotatedImageData)->Initial(height * width * m_BufPicNum, width * height);//此处默认为旋转90度，则图片的大小不变，否则需要根据angle计算旋转之后的图片大小
	//注册缓冲区
	...
	std::dynamic_pointer_cast<RotatedImageDataItem>(m_rotatedDataProvider)->registerOutputBuffer(0, m_cmosRotatedImageData);
	std::dynamic_pointer_cast<ZoomedImageDataItem>(m_zoomedDataProvider)->registerInputBuffer(0, m_cmosRotatedImageData);
	...
	//开始图像缩放
	m_zoomedDataProvider->setup(rotatedFeatures->attachedDataWidth, rotatedFeatures->payloadDataWidth, rotatedFeatures->linesPerFrame);
	const FeaturesOfDataItem* zoomedFeatures = m_zoomedDataProvider->constDataFeatures();

	//设置通道编号
	//m_magnifier->setCmosNumber(cmosId);
    //窗口绑定数据指针，初始化m_dataProvider
	m_window->setDataItemPtr(m_zoomedDataProvider);
	...
	m_magnifier->setDataItemPtr(m_zoomedDataProvider);
	//窗口绑定数据转换指针，初始化m_convertor
	...
	//设置窗口大小
	m_window->setMagnifierRange(zoomedFeatures->payloadDataWidth, zoomedFeatures->linesPerFrame);
	//定时刷新界面
